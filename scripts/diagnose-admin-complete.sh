...
docker exec $NGINX_CONTAINER sh -c "wget -q -O- --timeout=5 http://admin-dashboard:4200/ 2>&1" | head -30 || echo "❌ Connexion échouée"
echo ""
...
docker exec $NGINX_CONTAINER cat /etc/nginx/conf.d/default.conf 2>&1 | head -20 || echo "✅ Fichier vide ou inexistant (bon signe)"
echo ""
...
docker exec $NGINX_CONTAINER nginx -T 2>&1 | grep -B 5 -A 5 "default_server" || echo "Aucun default_server trouvé"
echo ""

echo "9. Tous les serveurs qui écoutent sur 443 :"
docker exec $NGINX_CONTAINER nginx -T 2>&1 | grep -B 10 "listen 443" || echo "Aucun serveur 443 trouvé"
echo ""

echo "10. Configuration complète de location /admin/ :"
docker exec $NGINX_CONTAINER nginx -T 2>&1 | grep -B 5 -A 20 "location /admin" || echo "location /admin/ non trouvé"
echo ""

echo "11. Logs d'erreur nginx (dernières 50 lignes) :"
docker exec $NGINX_CONTAINER tail -n 50 /var/log/nginx/error.log 2>&1
echo ""

echo "12. Logs d'accès pour /admin/ (dernières 20 lignes) :"
docker exec $NGINX_CONTAINER tail -n 100 /var/log/nginx/access.log 2>&1 | grep -i admin | tail -20 || echo "Aucun accès /admin/ trouvé"
echo ""
...
docker network inspect air_backend 2>&1 | grep -A 3 "admin-dashboard" || echo "admin-dashboard non trouvé sur le réseau"
echo ""
